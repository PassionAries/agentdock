...
func allToolSpecs() []ToolSpec {
	...
	return bindToolSchemas([]ToolSpec{
		...
		{Name: "browser_session", Title: "Browser session", Description: "Start, close, or clean up an AgentDock-owned Chrome, Chromium, or Edge session using the native Go CDP runtime. Persistent login is available through profile_id; Node.js and Playwright are not required.", Availability: requiresBrowser, Handler: ctxToolHandler((*Runtime).browserSession)},
		{Name: "browser_act", Title: "Browser actions", Description: "Run strictly validated CSS/CDP browser actions against an AgentDock-owned Chromium-family session and return the final typed page snapshot plus screenshot Artifact.", Availability: requiresBrowser, Handler: ctxToolHandler((*Runtime).browserAct)},
