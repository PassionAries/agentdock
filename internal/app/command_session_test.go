...
func TestExecCommandRejectsInvalidExecutionMode(t *testing.T) {
	...
	if runtime.command.Store().ReservationCount() != 0 || len(runtime.command.Store().List()) != 0 {
...
func TestListSessionsKeepsCompletedResultAvailable(t *testing.T) {
	...
	session, ok := runtime.command.Store().Get(sessionID)
	if !ok {
	...
	if _, ok := runtime.command.Store().Get(sessionID); ok {
...
func TestKillCompletedSessionReturnsActualStatus(t *testing.T) {
	...
	sessionID, _ := started["session_id"].(string)
	stored, ok := runtime.command.Store().Get(sessionID)
	if !ok {
		t.Fatalf("session %q was not stored", sessionID)
	}
	select {
...
func TestKillAllSessionsWaitsForEveryProcess(t *testing.T) {
	...
	for _, stored := range sessions {
		...
		if _, ok := runtime.command.Store().Get(stored.ID); ok {
...
func TestExecCommandRejectsWhenRunningSessionLimitIsReached(t *testing.T) {
	...
	for i := 0; i < maxConcurrentCommandSessions; i++ {
		rt.command.Store().Add(&session.Session{
...
func TestRuntimeCloseStopsRunningCommandSessions(t *testing.T) {
	...
	stored, ok := rt.command.Store().Get(sessionID)
	if !ok {
		t.Fatalf("session %q was not stored", sessionID)
	}
	...
	if _, ok := rt.command.Store().Get(sessionID); ok {
...
func TestRuntimeCloseCancelsCommandBeforeItIsStored(t *testing.T) {
	...
	for rt.command.Store().ReservationCount() == 0 && time.Now().Before(deadline) {
		time.Sleep(time.Millisecond)
	}
	if rt.command.Store().ReservationCount() == 0 {
	...
	if got := rt.command.Store().ReservationCount(); got != 0 {
		t.Fatalf("reservation count = %d, want 0", got)
	}
	if listed := rt.command.Store().List(); len(listed) != 0 {
