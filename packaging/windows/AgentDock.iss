...
chinesesimp.DocsShortcut=AgentDock 使用文档
chinesesimp.UninstallShortcut=卸载 AgentDock
chinesesimp.StartupPageCaption=启动方式
chinesesimp.StartupPageDescription=选择 AgentDock 如何随 Windows 启动
chinesesimp.StartupPageSubCaption=普通用户建议保持自动启动。AgentDock 始终只使用当前用户权限。
chinesesimp.StartupOption=登录 Windows 后自动启动 AgentDock 和托盘
chinesesimp.ConnectionPageCaption=连接方式
chinesesimp.ConnectionPageDescription=选择允许访问 AgentDock 的范围
chinesesimp.ConnectionPageSubCaption=默认只允许本机访问。公网访问必须由你明确开启。
chinesesimp.LocalMode=仅本机使用（推荐）
chinesesimp.QuickMode=创建临时公网地址
chinesesimp.NamedMode=使用自己的 Cloudflare 域名
chinesesimp.FixedPageCaption=固定公网地址
chinesesimp.FixedPageDescription=填写 Cloudflare Tunnel 信息
chinesesimp.FixedPageSubCaption=公网地址只填写 HTTPS Origin，例如 https://agent.example.com。Tunnel Token 不会进入命令行。
chinesesimp.ServerURLLabel=HTTPS 公网地址：
chinesesimp.TunnelTokenLabel=Cloudflare Tunnel Token：
chinesesimp.CopyLocal=复制本地 MCP 地址
chinesesimp.CopyPublic=复制公网 MCP 地址
chinesesimp.CopyBearer=复制 Bearer Token
chinesesimp.CopyOAuth=复制 OAuth 登录密码
chinesesimp.InvalidServerURL=固定公网地址必须以 https:// 开头，并且不能包含引号。
chinesesimp.TokenRequired=请填写 Cloudflare Tunnel Token。
chinesesimp.TokenFileFailed=无法创建临时 Tunnel Token 文件。
chinesesimp.InstallerStartFailed=无法启动 AgentDock 安装器。
chinesesimp.InstallerExitCode=安装脚本退出码：
chinesesimp.InstallFailed=AgentDock 安装失败：
chinesesimp.Health=状态：
chinesesimp.LocalMCP=本地 MCP 地址：
chinesesimp.PublicMCP=公网 MCP 地址：
chinesesimp.BearerToken=Bearer Token：
chinesesimp.OAuthPassword=OAuth 登录密码：
chinesesimp.PurgeStateQuestion=是否同时删除任务、Skill、配置和默认工作目录？选择“否”将保留用户数据。
...
Name: "{group}\{cm:DocsShortcut}"; Filename: "https://uvwt.github.io/agentdock-docs/"
...
begin
  StartupPage := CreateInputOptionPage(
    wpSelectDir,
    CustomMessage('StartupPageCaption'),
    CustomMessage('StartupPageDescription'),
    CustomMessage('StartupPageSubCaption'),
    False,
    False
  );
  StartupPage.Add(CustomMessage('StartupOption'));
  StartupPage.Values[0] := True;

  ConnectionPage := CreateInputOptionPage(
    StartupPage.ID,
    CustomMessage('ConnectionPageCaption'),
    CustomMessage('ConnectionPageDescription'),
    CustomMessage('ConnectionPageSubCaption'),
    True,
    False
  );
  ConnectionPage.Add(CustomMessage('LocalMode'));
  ConnectionPage.Add(CustomMessage('QuickMode'));
  ConnectionPage.Add(CustomMessage('NamedMode'));
  ConnectionPage.SelectedValueIndex := 0;

  FixedTunnelPage := CreateInputQueryPage(
    ConnectionPage.ID,
    CustomMessage('FixedPageCaption'),
    CustomMessage('FixedPageDescription'),
    CustomMessage('FixedPageSubCaption')
  );
  FixedTunnelPage.Add(CustomMessage('ServerURLLabel'), False);
  FixedTunnelPage.Add(CustomMessage('TunnelTokenLabel'), True);
  ...
  CopyLocalButton.Caption := CustomMessage('CopyLocal');
  ...
  CopyPublicButton.Caption := CustomMessage('CopyPublic');
  ...
  CopyBearerButton.Caption := CustomMessage('CopyBearer');
  ...
  CopyOAuthButton.Caption := CustomMessage('CopyOAuth');
...
begin
  ...
  begin
    ...
    begin
      MsgBox(CustomMessage('InvalidServerURL'), mbError, MB_OK);
      Result := False;
      Exit;
    end;
    ...
    begin
      MsgBox(CustomMessage('TokenRequired'), mbError, MB_OK);
      Result := False;
      Exit;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Result := CustomMessage('TokenFileFailed');
        Exit;
      end;
    end;
  ...
  begin
    Result := CustomMessage('InstallerStartFailed');
    Exit;
  end;
  if ExitCode <> 0 then
  begin
    ...
    if ErrorMessage = '' then
      ErrorMessage := CustomMessage('InstallerExitCode') + ' ' + IntToStr(ExitCode);
    Result := CustomMessage('InstallFailed') + ' ' + ErrorMessage;
  end;
end;
...
begin
  ...
  Summary := CustomMessage('Health') + ' ' +
    GetIniString('AgentDock', 'Health', 'unknown', ResultFilePath) + #13#10 +
    CustomMessage('LocalMCP') + ' ' + LocalMCPURL;
  if PublicMCPURL <> '' then
    Summary := Summary + #13#10 + CustomMessage('PublicMCP') + ' ' + PublicMCPURL;
  if BearerToken <> '' then
    Summary := Summary + #13#10 + CustomMessage('BearerToken') + ' ' + BearerToken;
  if OAuthPassword <> '' then
    Summary := Summary + #13#10 + CustomMessage('OAuthPassword') + ' ' + OAuthPassword;
...
begin
  ...
  if not UninstallSilent then
    PurgeState := MsgBox(
      CustomMessage('PurgeStateQuestion'),
      mbConfirmation,
      MB_YESNO
    ) = IDYES;
  Result := True;
